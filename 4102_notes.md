...
Let's approach this dynamically.  Start by defining x_m and y_m as the current character of a string M.  They either match, x_m is wrong, or y_m is wrong.  (This is optimal substructure by exchange argument.)  We then know that Optimal solution(i,j) = Opt(i,j) = delta_j, delta_i, or the minimum of (alpha_xy, delta + Opt(i-1, j), or delta + Opt(i, j-1)).  This complexity is Θ(m\*n) (duh, you're making a grid of height m and width n).  Note that to start you will need to set the values to 0 in the blank spaces before the words, 1 for the first letter, ..., n for the last letter of the word.  To convert one to another, backtrace through the graph to find the shortest path from each node.  This can be done greedily.
